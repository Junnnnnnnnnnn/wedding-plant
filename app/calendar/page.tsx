...
export default function CalendarPage() {
    ...
    const [calendarData, setCalendarData] = useState<Record<string, CalendarPlanItem[]>>({});
    ...
    const [selectedDateParams, setSelectedDateParams] = useState({ day: 0, month: 0, year: 0 });
    ...
    const fetchSchedules = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetchWithAuth(`/plan/schedule/calendar?${params.toString()}`);
            const json = await res.json();
            if (json.result === true && json.data?.list) {
                const byDay: Record<string, CalendarPlanItem[]> = {};
                for (const item of json.data.list as { day: string; list: CalendarPlanItem[] }[]) {
    ...
    const getSchedulesForDay = (day: number, m: number, y: number): ScheduleListItem[] => {
        let targetYear = y;
        let targetMonth = m;
        if (m === -1) { targetMonth = 11; targetYear -= 1; }
        else if (m === 12) { targetMonth = 0; targetYear += 1; }
    ...
    const handleDayClick = (dateObj: { day: number, month: number, year: number }) => {
        const dayPlans = getSchedulesForDay(dateObj.day, dateObj.month, dateObj.year);

        let targetYear = dateObj.year;
        let targetMonth = dateObj.month;
        if (dateObj.month === -1) { targetMonth = 11; targetYear -= 1; }
        else if (dateObj.month === 12) { targetMonth = 0; targetYear += 1; }

        setSelectedDateLabel(`${targetYear}년 ${targetMonth + 1}월 ${dateObj.day}일`);
        setSelectedDayPlans(dayPlans);
        setSelectedDateParams({ day: dateObj.day, month: targetMonth, year: targetYear });
        setIsModalOpen(true);
    };

    const weekdays = ["일", "월", "화", "수", "목", "금", "토"];

    const getAddPlanPath = (date?: { day: number; month: number; year: number }) => {
        const params = new URLSearchParams();
        if (roomId?.trim()) params.set("roomId", roomId.trim());
        params.set("from", "calendar");
        if (date) {
            params.set("date", `${date.year}-${String(date.month + 1).padStart(2, "0")}-${String(date.day).padStart(2, "0")}`);
        }
            ...
            <div className="h-full max-w-md mx-auto bg-white shadow-2xl relative overflow-hidden flex flex-col z-10">
                ...
                <div className="flex-1 overflow-y-auto w-full flex flex-col pb-24 scrollbar-hide">
                    ...
                    <div className="grid grid-cols-7 px-4 border-b border-gray-100 mb-2">
                        {weekdays.map((d, i) => (
                            <div key={d} className={`text-center py-2 text-xs font-bold ${i === 0 ? 'text-red-400' : i === 6 ? 'text-blue-400' : 'text-gray-400'}`}>
                                {d}
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="flex-1 grid grid-cols-7 px-4 content-start">
                        {daysInMonth.map((dateObj, idx) => {
                            const daySchedules = getSchedulesForDay(dateObj.day, dateObj.month, dateObj.year);
                            const isToday = getKstDate().getDate() === dateObj.day &&
                            ...
                            return (
                                ...
                                >
                                    <div className="flex justify-between items-center">
                                        <span className={`text-xs font-bold ${!dateObj.isCurrentMonth ? 'text-gray-300' :
                                            isToday ? 'bg-[#ee2b8c] text-white w-5 h-5 flex items-center justify-center rounded-full' :
                                    ...
                                    <div className="flex flex-col gap-0.5 overflow-hidden">
                                        {daySchedules.slice(0, 2).map(s => (
                                            <div
                                                key={s.id}
                                                className={`text-[8px] p-1 rounded-md truncate transition-colors ${s.status === 'COMPLETED'
                    ...
                    >
                        ...
                        >
                            <div className="flex items-center justify-between">
                                <h2 className="text-xl font-black text-[#1b0d14]">{selectedDateLabel}</h2>
                            ...
                            <div className="flex flex-col gap-3 overflow-y-auto max-h-[400px] scrollbar-hide">
                                {selectedDayPlans.length === 0 ? (
                                    ...
                                    selectedDayPlans.map(plan => (
                                        <button
                                            key={plan.id}
                                            onClick={() => router.push(`/schedule-detail?id=${plan.id}&from=calendar${roomId ? `&roomId=${roomId}` : ""}`)}
                                            className="flex items-center gap-4 bg-gray-50 p-4 rounded-2xl hover:bg-gray-100 transition-colors text-left"
                                        >
                                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${plan.status === 'COMPLETED' ? 'bg-gray-200' : 'bg-[#ee2b8c10]'}`}>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <p className={`font-bold text-[#1b0d14] truncate ${plan.status === 'COMPLETED' ? 'line-through text-gray-400' : ''}`}>
                                                    {plan.title}
                                                </p>
                                                <p className="text-xs text-gray-400 font-bold">{plan.categoryName}</p>
                                            </div>
                                ...
                                <button
                                    onClick={() => router.push(getAddPlanPath(selectedDateParams))}
