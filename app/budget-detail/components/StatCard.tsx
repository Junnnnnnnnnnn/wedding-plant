...
const StatCard: React.FC<StatCardProps> = ({
  ...
  return (
    <div
      className={`${containerPadding} rounded-[28px] flex flex-col gap-1.5 ${styles[variant]} transition-transform hover:scale-[1.01]`}
    >
      <span
        className={`${labelSize} font-extrabold uppercase tracking-[0.15em] ${labelStyles[variant]}`}
      >
        {label}
      </span>
      ...
      {remainingAmount !== undefined && (
        <p
          className={`mt-auto pt-2 text-right text-sm font-semibold ${labelStyles[variant]}`}
        >
