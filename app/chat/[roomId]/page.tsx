...
import { ChevronLeft, Send, Image as ImageIcon, Smile, MoreVertical, CheckCheck } from "lucide-react";
...
export default function ChatPage() {
    ...
    useEffect(() => {
        // room info fetch if needed
        const fetchRoomInfo = async () => {
            try {
                const res = await fetchWithAuth(`/plan/room/${roomId}`);
                const json = await res.json();
                if (json.result && json.data) {
                    setRoomName(`${json.data.onwerName}님의 웨딩 플랜`);
                }
            } catch (err) {
            ...
            <div className="h-full max-w-md mx-auto bg-white shadow-2xl relative overflow-hidden flex flex-col z-10">
                ...
                <header className="flex items-center justify-between px-4 h-16 bg-white/80 backdrop-blur-md border-b border-gray-100 flex-shrink-0 z-50">
                    <div className="flex items-center gap-3">
                        <button onClick={() => router.back()} className="p-1 -ml-1 text-stone-600">
                            <ChevronLeft className="w-6 h-6" />
                        </button>
                        <div>
                            <h1 className="text-base font-black text-[#1b0d14] max-w-[200px] truncate">{roomName}</h1>
                        ...
                        return (
                            <div key={msg.id} className={`flex ${isMe ? "justify-end" : "justify-start"} items-end gap-2`}>
                                ...
                                <div className={`flex flex-col ${isMe ? "items-end" : "items-start"} max-w-[70%]`}>
                                    {!isMe && showAvatar && (
                                        <span className="text-[10px] font-bold text-gray-400 mb-1 ml-1">{msg.senderName}</span>
                                    )}
                                    <div className={`px-4 py-2.5 rounded-[20px] text-sm font-medium shadow-sm leading-relaxed ${isMe
                                ...
                                <div className="flex flex-col items-center gap-0.5 mb-1 flex-shrink-0">
                                    {isMe && <CheckCheck className="w-3.5 h-3.5 text-[#ee2b8c]" />}
                                    <span className="text-[9px] font-bold text-gray-300">{msg.timestamp}</span>
                                </div>
                            </div>
                        );
                    })}
