...
- **시트에 새 값을 붙이려면 `OPEN_FIELDS` 를 먼저 고쳐야 한다.** 시각·메모는
  ...
  - **장소 줄과 지도 자리는 비워 두지 않는다.** 피드의 목록 카드는 장소가
    ...
    지도를 못 내는 이유는 셋이라 갈라 적는다 — 장소가 아예 없으면
    `장소를 등록하지 않은 일정이에요`, 적긴 했는데 카카오에서 고르지 않아
    좌표가 없으면 `지도에 표시할 수 없는 장소예요`, **좌표는 있는데 국외면**
    `해외라서 지도를 보여 줄 수 없어요`.
  - **카카오 지도는 국내 타일만 있다.** 국외 좌표를 주면 지도가 뜨긴 뜨는데
    타일이 전부 `white.png`(데이터 없음)라 **빈 흰 상자**가 된다 — 실제로
    푸꾸옥(베트남) 호텔이 백지로 떴다. **신혼여행은 해외가 기본이라 드문
    분기가 아니다.** `BragPlanSheet` 의 `KOREA_BOUNDS` 로 걸러 지도도
    카카오맵 링크도 내지 않는다(링크를 열어도 빈 지도다).
