...
- **시트에 새 값을 붙이려면 `OPEN_FIELDS` 를 먼저 고쳐야 한다.** 시각·장소·
  메모는 공개 범위 밖이라 서버가 아예 안 내려 준다. 카드가 이미 다섯 값을 다
  보여 주므로, 시트는 **같은 데이터를 다르게** 낸다 — 날짜를 요일까지 펴고,
  그 카테고리 소계에서 차지하는 몫(%)을 낸다. 몫이 카드에 없던 유일한
  값이고, 그게 이 시트를 여는 이유다.
