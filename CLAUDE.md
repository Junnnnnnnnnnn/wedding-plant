...
- **폰에서도 핀을 겁니다.** 예전에는 `<1020` 에서 핀을 풀어, 스크롤텔링이 히어로
  ...
  세로로 쌓여 내용이 화면보다 1.3~1.7배 커지기 때문인데, 지금은 **두 칸을 같은
  자리에 겹쳐 두고 스크롤로 갈아 끼웁니다**(`swapIn`/`swapOut`). 한 번에 한 창만
  보이니 높이가 절반이 되고, 바뀌는 게 같은 자리에서 일어나 대비도 더 삽니다.
...
- **모바일 크기를 키울 때는 반드시 한 화면에 들어가는지 재세요.** `.pin2` 의
  `overflow:hidden` 이 위아래를 잘라 먹습니다. 14개 뷰포트(2327~320)로 확인합니다.
