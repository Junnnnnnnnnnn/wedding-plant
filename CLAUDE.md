...
- 카테고리 줄을 누르면 아래 항목 목록이 그 카테고리로 좁혀집니다. `필터 해제`
  는 표 제목 줄에 함께 둡니다.
- **AI 조언은 도넛 카드 안의 조용한 알약입니다.** 예전의 전폭 보라→분홍
  그라데이션 버튼은 화면에서 제일 시끄러운데 제일 안 중요했습니다.
...
- 가이드 앵커 5개(`#budget-stat-grid`·`#budget-ai-insight`·`#budget-analysis`·
  `#budget-tab`·`#budget-list`)를 건드리지 마세요. 이름은 예전 구조에서 온
  것이라 지금 붙어 있는 곳과 뜻이 조금 다릅니다 —
  `budget-stat-grid`=도넛 카드, `budget-ai-insight`=AI 알약,
  `budget-analysis`=카테고리 표, `budget-tab`=예정/사용 알약,
  `budget-list`=항목 목록.
...
- 하단 `공지 사항 및 소개` 는 아직 **아무 데도 가지 않는 문구**입니다.
  연결할 페이지가 생기면 링크로 바꾸고, 안 생기면 빼는 편이 낫습니다.
...
- **`/setting`** (온보딩) — 레일·탭바를 달지 않습니다. 온보딩 중에 홈·보드로 새면 안 됩니다.
  ...
  **몇 개나 더 묻는지가 처음부터 보여야 이탈이 줄어듭니다** — 예전에는 1686px 화면에
  ...
  - 패널은 **질문 단계에만** 붙습니다(`stepIndex` 1~4). **축하·환영·출입증(Lanyard)은
    전체 화면 연출이라 예전 그대로 두었습니다** — 단계로 세지도 않습니다.
    `Lanyard` 는 컨테이너가 바뀌면 물리 월드가 다시 잡히므로 특히 건드리지 마세요.
  - **`<1024` 는 한 픽셀도 바뀌지 않았습니다.** 새로 붙인 클래스는 전부 `lg:` 이고,
...
- `scripts/onboarding.cjs` — `/setting` 온보딩 4단계(날짜→예산→이름→약관)를 실제로 눌러 넘기며 폭별 캡처. `≥1024` 에서만 좌측 진행 패널이 붙는지, 연출 화면에는 안 붙는지 확인합니다
