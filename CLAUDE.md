...
- `scripts/onboarding.cjs` — `/setting` 온보딩 4단계(날짜→예산→이름→약관)를 실제로 눌러 넘기며 폭별(375·768·1280·1686·2327) 캡처. `≥1024` 에서만 좌측 진행 패널이 붙는지, 연출 화면에는 안 붙는지, **출입증 캔버스가 `≥1024` 에서 뷰포트 폭을 다 쓰는지**(그 아래는 폰 프레임 500px) 확인합니다. Lanyard 는 dynamic import 라 캔버스가 붙을 때까지 기다린 뒤 찍습니다
