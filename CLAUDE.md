...
### 들어오는 길은 카카오 로그인 하나입니다 (게스트 모드 없음)

**앱 화면도 온보딩도 로그인해야 들어갑니다** — `app/components/GuestGate.tsx`
(레이아웃에 마운트, `AuthRedirectToMain` 과 같은 자리).

- 막는 곳: `/main`·`/calendar`·`/plan-list`·`/feed`·`/brag`·`/budget-detail`·
  `/user`·`/add-plen`·`/schedule-detail`·`/chat` → 토큰이 없으면 **`/login`**
- 온보딩 `/setting` → 토큰이 없으면 **`/`(랜딩)**. 앱을 아직 모르는 사람이라
  "다시 오셨네요" 라고 말하는 로그인 화면보다 랜딩이 맞습니다. 그리고 로그인
  전에는 **답을 저장할 곳이 없습니다** — 예전에는 여기서 받은 값을
  sessionStorage 에 담아 뒀습니다.
- 안 막는 곳: `/`·`/login`(들어오는 문), `/privacy`(로그인 없이 봐야 하는 문서),
...
**게이트에서 복귀 경로를 저장하지 마세요.** 로그인 후 목적지는 복귀 경로가
"신규 사용자 → `/setting`" 보다 앞섭니다. 처음 온 사람이 주소로 `/calendar` 를
치고 로그인하면 온보딩을 건너뛴 빈 앱에 떨어집니다. 세션이 끊겨서 나간 경우의
복귀 경로는 `ApiContext` 가 401 을 받을 때 이미 적어 둡니다.

**"로그인 없이 둘러보기" 는 없앴습니다.** 랜딩 CTA 아래 링크와 `AuthButtons` 의
버튼이 입구였습니다 — 다시 만들지 마세요. 코드에는 게스트 시절의 흔적이
남아 있습니다(`lib/guestSchedule.ts`, `HAS_COMPLETED_GUEST_SETTING_KEY`,
`plan_guest_agreement`, 화면마다의 `!getToken()` 분기). **`KakaoLoginAlert` 의
이관 처리는 그대로 두세요** — 바뀌기 전 세션에 게스트 데이터가 남은 사람이
로그인하면 그 값을 계정으로 옮겨 줍니다.

로그인 후 목적지 분기 우선순위는 `KakaoLoginAlert` 의 `resolveDestination()`
한 곳입니다: **shareCode → returnPath → 플랜 완성된 기존 사용자 → 참여 방 보유
→ 신규 사용자(`/setting`)**. 각 분기는 반드시 `return` 으로 끝내야 뒤 분기가
잘못 트리거되지 않습니다.
...
**화면이 `onTabClick` 을 직접 넘길 때도 목적지는 `TAB_ROUTES` 에서 가져오세요.**
경로를 그 화면에 다시 적으면 탭이 늘거나 경로가 바뀔 때 그 화면만 옛 목적지에
남습니다 — 실제로 `/main` 은 `feed` 가 삼항 끝의 `: "/main"` 으로 떨어져 **홈에서
피드를 눌러도 아무 일도 일어나지 않았고**, `/calendar` 는 `feed` 분기가 아예 없어
클릭이 통째로 먹혔습니다(둘 다 피드가 "준비중" 모달이던 시절의 잔재).
확인은 `node scripts/tabbar-routes.cjs` — 다섯 화면에서 네 탭을 전부 눌러 봅니다.
...
- **`/setting`** (온보딩) — 레일·탭바를 달지 않습니다. 온보딩 중에 홈·보드로 새면 안 됩니다.
  - **로그인한 사람만 옵니다**(`GuestGate`). 단계는 `결혼 날짜 · 예산 · 이름 ·
약관 동의 · 함께할 사람` 5개 하나뿐이고 `ONBOARDING_STEPS` 에 있습니다 —
    진행 막대·`N / M 단계` 가 여기서 나옵니다. 예전에는 게스트용 4단계 배열이
    따로 있었습니다(방이 없어 공유 코드가 안 나와 `함께할 사람` 을 뺐습니다).
  ...
  - 약관 버튼은 `다음` 입니다(뒤에 초대가 남았습니다). 마지막
    `계획 짜러 가기` 는 초대 단계에 있습니다.
...
- **랜딩 안의 앱 목업은 지금 앱 화면을 그대로 옮긴 것입니다.** 앱 화면을 고치면
  여기도 같이 고쳐야 합니다 — 안 고치면 랜딩이 예전 앱을 광고합니다(실제로
  한동안 흰 머리글 + D-day 알약 + 파스텔 아이콘 타일 시절 화면이 걸려 있었습니다).
  - 홈 폰(`.apphead`·`.appbudget`·`.applist`·`.appcard`) = `/main` 모바일 —
    분홍 머리 면 + 얇은 예산 줄 + `이번 달에 할 일` + 회색 채움 카드 + 분홍 칩
  - 웹(`.wbar`·`.wcards`·`.wcols`) = `/main` 대시보드 — 상단 바 + 이번 달 할 일
    스트립 + 예산·다가오는 일정 패널
  - 채팅(`.chead`) = `/chat` 의 분홍 머리 면, `.phero`·`.pdetail` = 폰의
    `/schedule-detail`. **`.insp` 는 넓은 화면 인스펙터라 따로입니다**(노션 대조
    섹션에서 씁니다) — 둘을 합치지 마세요.
  - 앱 색은 `--app-fill`·`--app-ink-*`·`--app-chip-*` 토큰에 모아 두었습니다.
  - **목업을 키우면 핀 높이를 넘깁니다.** `scripts/landing-widths.cjs` 가
    `1100~1440 x 900` 에서 "핀 넘침"으로 잡습니다 — 키운 만큼 다른 데서 빼세요.
  - 클래스 이름이 겹치기 쉽습니다. 히어로의 `.tile` 과 부딪쳐 상세 아이콘이
    세로로 늘어난 적이 있어 그 자리는 `.ptile` 입니다.
...
- `scripts/auth-required.cjs` — **로그인 필수(게스트 모드 없음).** 토큰 없이 앱 화면 열 곳을 치면 `/login` 으로, `/setting` 은 랜딩으로 가는지, **게이트가 복귀 경로를 저장하지 않는지**(저장하면 신규 사용자가 온보딩을 건너뛴다), 예전 게스트 플래그가 남아 있어도 막는지, 문·문서·초대(`/`·`/login`·`/privacy`·`/share/…`·`?share=`)는 안 막는지, 랜딩·로그인에 "로그인 없이 둘러보기" 가 되살아나지 않았는지를 봅니다. `npm run dev` 만 있으면 되고 `HEADED=1` 로 띄워 볼 수 있습니다 (예전 `guest-flow.cjs` 자리)
...
- `scripts/onboarding.cjs` — `/setting` 온보딩을 실제로 눌러 넘기며 폭별(375·768·1280·1686·2327) 캡처. **로그인한 사람의 5단계 하나만 돕니다**(게스트 4단계는 게스트 모드와 함께 없앴습니다) — 토큰을 심고 `share-code` 를 목으로 주며, 약관까지 실제로 동의해 넘어간 뒤 초대 단계에서 카드 선택 전 버튼 잠김 · 선택 후 라벨 전환 · 보낸 뒤 안내 문구 · **보낸 링크의 `?as=spouse`** · **`POST /plan/setting` 이 `share-code` 보다 먼저 나갔는지** · 끝나고 `/main` 으로 갔는지까지 확인합니다. 헤드리스 크롬에도 `navigator.share` 가 있어 그대로 두면 취소로 빠지므로 하네스가 스텁합니다. 기본은 다섯 폭을 전부 돌고 `WIDTHS` 를 주면 그 폭만 씁니다. `≥1024` 에서만 좌측 진행 패널이 붙는지, 연출 화면에는 안 붙는지, **출입증 캔버스가 `≥1024` 에서 뷰포트 폭을 다 쓰는지**(그 아래는 폰 프레임 500px) 확인합니다. Lanyard 는 dynamic import 라 캔버스가 붙을 때까지 기다린 뒤 찍습니다
...
- `scripts/tabbar-routes.cjs` — **하단 탭바 라우팅 전수.** 다섯 화면(`/main`·`/calendar`·`/feed`·`/plan-list`·`/user`)에서 네 탭을 모두 눌러 제 목적지로 가는지 봅니다. 화면마다 `onTabClick` 을 따로 적어 두면 그 화면만 옛 목적지에 남는데, 그걸 잡습니다
