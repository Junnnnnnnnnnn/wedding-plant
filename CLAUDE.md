...
- **모바일(<1020)에는 핀이 없습니다.** 그래서 무대 진행도(`--st`)에 묶인 안무는
  한 번에 지나가 버립니다. 그 폭에서는 `animation-timeline: view()` 로 갈아
  **각 덩어리가 화면에 들어올 때 떠오르게** 합니다. 넓은 화면용 비율도 같이
  풀어야 합니다 — 노션 창은 사이드바를 접고 표에 폭을 주고, `.win2` 의
  `min-height` 를 0 으로 두지 않으면 카드 아래가 통째로 빕니다.
